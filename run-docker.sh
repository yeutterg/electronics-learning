...
CONTAINER_ID=`docker run --rm -p 8888:8888 -d -v /$PWD:/electronics-courses electronics-courses tail -f /dev/null`
...
if [ "$JUPYTER" = true ]; then
    ...
    docker exec -ti $CONTAINER_ID jupyter notebook --ip=0.0.0.0 --port=8888 --allow-root --no-browser /electronics-courses/
else
